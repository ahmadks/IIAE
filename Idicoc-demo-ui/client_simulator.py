...
from idicoc_notary_core import FilePolicyLoader, IDICOCNotaryClient
import IdicocConfg
...
def load_policies_from_file(file_path: str) -> list[dict]:
    if not os.path.exists(file_path):
        return []
    return FilePolicyLoader(file_path).load_policies()
...
def main():
    parser = argparse.ArgumentParser(
        description="IIAE Notary Headless Client Simulator"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Sigma para ruido numérico, o tasa de error para texto",
    )
    parser.add_argument(
        "--interval",
        type=float,
        default=1.0,
        help="Intervalo en segundos entre señales",
    )
    parser.add_argument(
        "--text",
        type=str,
        default="Transfer 50000.00 euros within limits.",
        help="Texto base para modo Semántico",
    )
    ...
    if args.mode == "Semantico":
        policies = [
            ax for ax in static_policies if ax.get("mode", "all") in ("semantic", "all")
        ]
    else:
        policies = [
            ax for ax in static_policies if ax.get("mode", "all") in ("numeric", "all")
        ]

    print(
        f"[*] Inicializando IDICOC Notary en modo {args.mode} (Epsilon={args.epsilon})..."
    )
    ...
    try:
        while True:
            if args.mode == "Numerico":
                ...
                noise = (
                    np.random.normal(0, args.noise, 4)
                    if args.noise > 0
                    else np.zeros(4)
                )
                ...
                print(
                    f"[>] Enviando señal numérica: {[round(v, 4) for v in audit_input]}"
                )
            else:
            ...
            admitted = result.metadata.get("admission_metrics", {}).get(
                "admitted", False
            )

            if admitted:
                status = (
                    "\033[92mADMITIDA\033[0m"
                    if not correction
                    else "\033[93mCORREGIDA\033[0m"
                )
            else:
                status = "\033[91mRECHAZADA\033[0m"

            print(
                f"[<] Notario Responde: {status} | D_s: {D_s:.4f} | Epsilon: {args.epsilon}"
            )
            print("-" * 60)
