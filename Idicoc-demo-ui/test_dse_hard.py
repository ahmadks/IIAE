...
def run_test_scenario(name, audit_input, policies, expected_d_2_inf=False, expected_status="REJECTED"):
    print(f"\n{'='*50}\n[TEST] {name}\n{'='*50}")
    ...
    if expected_d_2_inf:
        ...
        assert math.isinf(d_s), f"Fallo: se esperaba D_s = inf, pero se obtuvo {d_s}"
    else:
        assert not math.isinf(d_2), f"Fallo: no se esperaba d_2 = inf, pero se obtuvo {d_2}"
        assert not math.isinf(d_s), f"Fallo: no se esperaba D_s = inf, pero se obtuvo {d_s}"
        
    if expected_status == "REJECTED":
        assert status == "REJECTED" or corr_flag is True, "Fallo: se esperaba que la señal fuera rechazada."
    elif expected_status == "ADMITIDO":
        assert status == "ADMITIDO" and corr_flag is False, "Fallo: se esperaba que la señal fuera admitida."
...
def main():
    ...
    run_test_scenario(
        ...
        policies=[{
            "id": "p1", "text": "prohibido mentir", "policy_type": "semantic",
            "polarity": "affirmative", "hardness": "hard"
        }],
    ...
    run_test_scenario(
        ...
        policies=[{
            "id": "p2", "text": "el modelo siempre es preciso", "policy_type": "semantic",
            "polarity": "affirmative", "hardness": "soft"
        }],
        expected_d_2_inf=False,
        expected_status="REJECTED" # Assuming the distance > 0.20
    )
    ...
    run_test_scenario(
        ...
        policies=[{
            "id": "p3", "text": "0.9", "policy_type": "regex",
            "polarity": "negative", "hardness": "hard"  # Negative polarity = rechazar si hay match
        }],
        expected_d_2_inf=True,
        expected_status="REJECTED"
    )
    ...
    run_test_scenario(
        ...
        policies=[{
            "id": "p4", "text": "0.5", "policy_type": "regex",
            "polarity": "negative", "hardness": "soft" 
        }],
        expected_d_2_inf=False,
        expected_status="REJECTED" # It will get a finite penalty, but enough to trigger correction
    )
    ...
    run_test_scenario(
        ...
        policies=[{
            "id": "p5", "text": "error fatal", "policy_type": "regex",
            "polarity": "negative", "hardness": "hard"
        }],
        expected_d_2_inf=False,
        expected_status="ADMITIDO"
    )
