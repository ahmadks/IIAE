...
def generate_merkle_root(axioms: list, salt: str = None) -> str:
    ...
    hashes = [
        hashlib.sha256((str(ax) + _salt).encode("utf-8")).hexdigest() for ax in axioms
    ]
    ...
    while len(hashes) > 1:
        ...
        for i in range(0, len(hashes), 2):
            ...
            next_level.append(
                hashlib.sha256((combined + _salt).encode("utf-8")).hexdigest()
            )
        hashes = next_level
...
def create_receipt(
    ...
    return {"payload": payload, "ctm_seal": ctm_seal, "axioms": axioms}
