...
# New modular structure (non-breaking additions)
from . import core
from . import enterprise
from . import pipeline_debug
...
def log_audit_record(record: dict, config: IIAEConfig | None = None) -> None:
    """Send a structured audit record to the configured log destination.

    Destination is controlled by ``IIAEConfig.log_destination`` / env
    ``IIAE_LOG_DESTINATION`` — not by the CTM.
    """
