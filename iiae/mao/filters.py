"""MAO filter functions moved from legacy module to dedicated filters module.
These functions are used by the LexicalMAOEngine implementation.
"""
import re
...
def axiomatic_invariance_filter(axioms: list, response: str) -> dict:
    ...
    for ax in axioms:
        ...
        if any(neg in response_clean and neg not in ax_clean for neg in negations):
            return {"passed": False, "score": None, "reason": f"Axiomatic boundary violation detected for: '{ax}'"}
    return {"passed": True, "score": 1.0, "reason": None}
...
def probability_filter(response: str) -> dict:
    ...
    return {"passed": entropy_score >= 0.6, "score": max(0.0, entropy_score), "reason": None}
