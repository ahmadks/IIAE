...
class PhiProvider(BaseLLMProvider):
    """Provider wrapper for Phi-style local models.
    ...
    def __init__(self, model_path: str | None = None, embedding_model_name: str | None = None):
        self.model_path = model_path or "models_cache/Phi-3.5-mini-instruct"
    ...
    def _ensure_model(self) -> None:
        ...
        if not is_directory:
            try:
                ...
                if self.model_path:
                    ...
                    raise RuntimeError("No model_path provided for PhiProvider")
        ...
        try:
            ...
            print(f"[PhiProvider] Cargando modelo transformers desde {self.model_path}...")
            ...
            print(f"[PhiProvider] Modelo transformers cargado exitosamente en {device}.")
        except Exception as e:
            raise ImportError(f"Unable to initialize Phi model via transformers or llama-cpp: {e}")

    def generate(self, prompt: str, logits_processor: Any = None) -> str:
        self._ensure_model()
        try:
            ...
            if hasattr(self._model, "generate"):
                ...
                gen_kwargs = {
                    "max_new_tokens": 80,  # Reduced for faster demo generation
                    "do_sample": False,    # Greedy decoding avoids multinomial sampling bugs on MPS and is much faster
            ...
            raise RuntimeError(f"PhiProvider.generate failed: {e}")

    def get_embedding(self, text: str) -> list[float]:
        if self.embedding_provider is None:
            raise ImportError("No embedding model available in PhiProvider")
