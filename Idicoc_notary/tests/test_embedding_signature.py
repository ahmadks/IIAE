...
def test_signature_deterministic():
    sig1 = compute_embedding_signature("modelA", normalize=True)
    sig2 = compute_embedding_signature("modelA", normalize=True)
    assert sig1 == sig2

def test_signature_differs_on_model():
    sig1 = compute_embedding_signature("modelA", normalize=True)
    sig2 = compute_embedding_signature("modelB", normalize=True)
    assert sig1 != sig2

def test_signature_differs_on_normalize():
    sig1 = compute_embedding_signature("modelA", normalize=True)
    sig2 = compute_embedding_signature("modelA", normalize=False)
    assert sig1 != sig2
