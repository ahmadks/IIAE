...
def test_compute_dissonance_with_property_graph(mock_d1_vec, mock_to_vector, strategy):
    ...
    """
    # Mocking embeddings
    mock_to_vector.side_effect = lambda text, **kw: (
        np.array([1.0, 0.0]) if text == "ancla" else np.array([0.0, 1.0])
    )
...
def test_pipeline_graph_integration(mock_evaluate):
    ...
    res = auditor.execute_audit(
        user_prompt="test audit", rag_context="context chunk", llm_output="test output"
    )
