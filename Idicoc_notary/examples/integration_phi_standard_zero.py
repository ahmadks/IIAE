"""
Ejemplo de Integración: IIAE con Phi-3.5-mini (Phases 1-4)
...
- CUDA / MPS (recomendado, pero no obligatorio)

Ejecutar:
    python examples/integration_phi_standard_zero.py
"""
...
def phase_1_cold_loop():
    ...
    print("\n[Fase 1] Asegurando el modelo Phi en cache local...")
    try:
        ...
        print(f"[Fase 1] ⚠ No se pudo descargar Phi automáticamente: {exc}")
        print(
            "  Asegúrate de descargar el modelo de Hugging Face manualmente si no tienes conexión a internet."
        )

    print("\n[Fase 1] Inicializando AuditConfig...")
    config = AuditConfig(
        policy_file_path=policies_path,
        llama_model_name="microsoft/Phi-3.5-mini-instruct",
...
def phase_3_hot_loop_mock(config: AuditConfig, context_input: list, user_input: str):
    ...
    Simula generación con Phi + DeterministicMUXLogitsProcessor.
    ...
    try:
        ...
        print(f"\n[Fase 3] Cargando Phi tokenizador...")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("IIAE - EJEMPLO INTEGRACIÓN COMPLETO (PHI-3.5)")
    print("=" * 70)
