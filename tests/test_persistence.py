...
from idicoc_core.core.custody.merkle_dag import CustodialTraceManager, MerkleDAG
...
def test_pipeline_with_persistence():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        wrapper = IDICOCWrapper(config, analyzer, aem_storage=aem_storage, ctm_storage=ctm_storage)
        ...
        wrapper2 = IDICOCWrapper(config, analyzer, aem_storage=aem_storage2, ctm_storage=ctm_storage2)
        
        # The reloaded MerkleDAG must have the exact same root hash and nodes
        assert wrapper2.pipeline.runtime_config.ctm.root_hash == root_hash_1
        assert len(wrapper2.pipeline.runtime_config.aem.entropy_map["DISCARDED_NOISE"]) == 1

def test_ctm_modes():
    config_log = AuditConfig(audit_mode="mathematical", rigidity_epsilon=0.5, ctm_mode="log_only")
    analyzer = BankEntropyAnalyzer()
    wrapper_log = IDICOCWrapper(config_log, analyzer)
    ...
    wrapper_dis = IDICOCWrapper(config_dis, analyzer)
